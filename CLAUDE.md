...
从零实现简易版 Claude Code（学习型 agent 工具）。核心：自研迷你 IoC 容器、可插拔 LLM、`@Tool` 工具系统、agent 循环、多界面适配（CLI 先行，Web 二期）。v1（CLI）已完成，当前进入 v2 阶段（Hook → 权限 → Memory → Skill → Planning → Subagent → Web）。
- 根包：`com.learn.mycc`
...
| v1 开发计划（已完成） | `docs/plans/2026-08-27-v1-development-plan.md` |
| v2 开发计划（进行中） | `docs/plans/2026-09-03-v2-development-plan.md` |
| 开发日志 | `dev-log/`（每工作日一个 `YYYY-MM-DD.md`） |

## 工作说明（硬性纪律）

1. **增量推进**：严格按开发计划分阶段执行——v1 M1→M6 已完成；v2 M7→M13 进行中。一个阶段完成、测试通过、人工检查点确认后再进入下一阶段；不提前实现未排期功能（YAGNI）。
...
mycc-core(机制层: IoC/注解/ToolRegistry) · mycc-ui(仅 UI 接口: InteractionPort/OutputEvent)
  └─ mycc-ai(LLM) · mycc-tools(内置工具) · mycc-storage(存储)   ← 依赖 core
        └─ mycc-agent(agent 循环，只依赖接口，含 mycc-ui)
              ├─ mycc-cli(CLI 渲染，实现 mycc-ui) · mycc-web(v2，M13 构建) ← 具体 UI 实现
